...
public class DeviceCreateSixnetEthertrakCommandSettings : DeviceCreateCommandBaseSettings
{
    [CommandOption("--device-id <DEVICE-ID>")]
    [Description("Device IP address (e.g., 192.168.1.1 or 192.168.1.1.5 for RS-485 bridging)")]
    public string DeviceId { get; init; } = "255.255.255.255";

    [CommandOption("--port")]
    [Description("TCP/IP port that the remote device is configured to use (0-65535)")]
    [DefaultValue(502)]
    public int PortNumber { get; init; }

    [CommandOption("--output-discretes")]
    [Description("Number of output coils (bits) to read in one request (8-800)")]
    [DefaultValue(32)]
    public int OutputDiscretes { get; init; }

    [CommandOption("--input-discretes")]
    [Description("Number of input coils (bits) to read in one request (8-800)")]
    [DefaultValue(32)]
    public int InputDiscretes { get; init; }

    [CommandOption("--output-registers")]
    [Description("Number of output registers (words) to read in one request (1-120)")]
    [DefaultValue(32)]
    public int OutputRegisters { get; init; }

    [CommandOption("--input-registers")]
    [Description("Number of input registers (words) to read in one request (1-120)")]
    [DefaultValue(32)]
