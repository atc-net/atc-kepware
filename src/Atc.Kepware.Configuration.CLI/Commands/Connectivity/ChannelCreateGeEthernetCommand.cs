...
public sealed class ChannelCreateGeEthernetCommand : AsyncCommand<ChannelCreateGeEthernetCommandSettings>
{
    private readonly ILoggerFactory loggerFactory;
    private readonly ILogger<ChannelCreateGeEthernetCommand> logger;
    private readonly IKepwareConfigurationClient kepwareConfigurationClient;

    public ChannelCreateGeEthernetCommand(
        ILoggerFactory loggerFactory,
        IKepwareConfigurationClient kepwareConfigurationClient)
    {
        this.loggerFactory = loggerFactory;
        logger = loggerFactory.CreateLogger<ChannelCreateGeEthernetCommand>();
        this.kepwareConfigurationClient = kepwareConfigurationClient;
    }

    public override Task<int> ExecuteAsync(
        CommandContext context,
        ChannelCreateGeEthernetCommandSettings settings)
    {
        ...
        ChannelCreateGeEthernetCommandSettings settings)
    {
        ...
        try
        {
            ...
            var request = BuildGeEthernetChannelRequest(settings);
            var result = await kepwareConfigurationClient.CreateGeEthernetChannel(request, CancellationToken.None);
    ...
    private static GeEthernetChannelRequest BuildGeEthernetChannelRequest(
        ChannelCreateGeEthernetCommandSettings settings)
        => new()
        {
            Name = settings.Name,
