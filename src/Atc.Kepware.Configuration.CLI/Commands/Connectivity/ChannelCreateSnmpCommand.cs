...
public sealed class ChannelCreateSnmpCommand : AsyncCommand<ChannelCreateSnmpCommandSettings>
{
    private readonly ILoggerFactory loggerFactory;
    private readonly ILogger<ChannelCreateSnmpCommand> logger;
    private readonly IKepwareConfigurationClient kepwareConfigurationClient;

    public ChannelCreateSnmpCommand(
        ILoggerFactory loggerFactory,
        IKepwareConfigurationClient kepwareConfigurationClient)
    {
        this.loggerFactory = loggerFactory;
        logger = loggerFactory.CreateLogger<ChannelCreateSnmpCommand>();
        this.kepwareConfigurationClient = kepwareConfigurationClient;
    }

    public override Task<int> ExecuteAsync(
        CommandContext context,
        ChannelCreateSnmpCommandSettings settings)
    {
        ...
        ChannelCreateSnmpCommandSettings settings)
    {
        ...
        try
        {
            ...
            var request = BuildSnmpChannelRequest(settings);
            var result = await kepwareConfigurationClient.CreateSnmpChannel(request, CancellationToken.None);
    ...
    private static SnmpChannelRequest BuildSnmpChannelRequest(
        ChannelCreateSnmpCommandSettings settings)
        => new()
        {
            Name = settings.Name,
