...
public sealed class ChannelCreateSattBusEthernetCommand : AsyncCommand<ChannelCreateSattBusEthernetCommandSettings>
{
    private readonly ILoggerFactory loggerFactory;
    private readonly ILogger<ChannelCreateSattBusEthernetCommand> logger;
    private readonly IKepwareConfigurationClient kepwareConfigurationClient;

    public ChannelCreateSattBusEthernetCommand(
        ILoggerFactory loggerFactory,
        IKepwareConfigurationClient kepwareConfigurationClient)
    {
        this.loggerFactory = loggerFactory;
        logger = loggerFactory.CreateLogger<ChannelCreateSattBusEthernetCommand>();
        this.kepwareConfigurationClient = kepwareConfigurationClient;
    }

    public override Task<int> ExecuteAsync(
        CommandContext context,
        ChannelCreateSattBusEthernetCommandSettings settings)
    {
        ...
        ChannelCreateSattBusEthernetCommandSettings settings)
    {
        ...
        try
        {
            ...
            var request = BuildSattBusEthernetChannelRequest(settings);
            var result = await kepwareConfigurationClient.CreateSattBusEthernetChannel(request, CancellationToken.None);
    ...
    private static SattBusEthernetChannelRequest BuildSattBusEthernetChannelRequest(
        ChannelCreateSattBusEthernetCommandSettings settings)
        => new()
        {
            Name = settings.Name,
