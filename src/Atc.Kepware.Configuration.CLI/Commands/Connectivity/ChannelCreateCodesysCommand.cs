...
public sealed class ChannelCreateCodesysCommand : AsyncCommand<ChannelCreateCodesysCommandSettings>
{
    private readonly ILoggerFactory loggerFactory;
    private readonly ILogger<ChannelCreateCodesysCommand> logger;
    private readonly IKepwareConfigurationClient kepwareConfigurationClient;

    public ChannelCreateCodesysCommand(
        ILoggerFactory loggerFactory,
        IKepwareConfigurationClient kepwareConfigurationClient)
    {
        this.loggerFactory = loggerFactory;
        logger = loggerFactory.CreateLogger<ChannelCreateCodesysCommand>();
        this.kepwareConfigurationClient = kepwareConfigurationClient;
    }

    public override Task<int> ExecuteAsync(
        CommandContext context,
        ChannelCreateCodesysCommandSettings settings)
    {
        ...
        ChannelCreateCodesysCommandSettings settings)
    {
        ...
        try
        {
            ...
            var request = BuildCodesysChannelRequest(settings);
            var result = await kepwareConfigurationClient.CreateCodesysChannel(request, CancellationToken.None);
    ...
    private static CodesysChannelRequest BuildCodesysChannelRequest(
        ChannelCreateCodesysCommandSettings settings)
        => new()
        {
            Name = settings.Name,
            ...
            KeepAliveMinutes = settings.KeepAliveMinutes,
        };
}
