...
public sealed class ChannelCreateMitsubishiFxNetCommand : AsyncCommand<ChannelCreateMitsubishiFxNetCommandSettings>
{
    private readonly ILoggerFactory loggerFactory;
    private readonly ILogger<ChannelCreateMitsubishiFxNetCommand> logger;
    private readonly IKepwareConfigurationClient kepwareConfigurationClient;

    public ChannelCreateMitsubishiFxNetCommand(
        ILoggerFactory loggerFactory,
        IKepwareConfigurationClient kepwareConfigurationClient)
    {
        this.loggerFactory = loggerFactory;
        logger = loggerFactory.CreateLogger<ChannelCreateMitsubishiFxNetCommand>();
        this.kepwareConfigurationClient = kepwareConfigurationClient;
    }

    public override Task<int> ExecuteAsync(
        CommandContext context,
        ChannelCreateMitsubishiFxNetCommandSettings settings)
    {
        ...
        ChannelCreateMitsubishiFxNetCommandSettings settings)
    {
        ...
        try
        {
            ...
            var request = BuildMitsubishiFxNetChannelRequest(settings);
            var result = await kepwareConfigurationClient.CreateMitsubishiFxNetChannel(request, CancellationToken.None);
    ...
    private static MitsubishiFxNetChannelRequest BuildMitsubishiFxNetChannelRequest(
        ChannelCreateMitsubishiFxNetCommandSettings settings)
        => new()
        {
            Name = settings.Name,
